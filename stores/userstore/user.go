...
func (u *userImpl) GetByID(ctx context.Context, id int) (e usermodel.Entity, err error) {
	if err = db.DBFromCtxValue(ctx, u.db).GetContext(ctx, &e, `SELECT id, name FROM t_user WHERE id = $1`, id); err != nil {
...
func (u *userImpl) GetByName(ctx context.Context, name string) (e usermodel.Entity, err error) {
	if err = db.DBFromCtxValue(ctx, u.db).GetContext(ctx, &e, `SELECT id, name FROM t_user WHERE name = $1`, name); err != nil {
...
func (u *userImpl) GetFirst(ctx context.Context) (e usermodel.Entity, err error) {
	if err = db.DBFromCtxValue(ctx, u.db).GetContext(ctx, &e, `SELECT id, name FROM t_user ORDER BY id ASC LIMIT 1`); err != nil {
		err = errors.WithStack(err)
		return
	}
...
func (u *userImpl) VerifyByNameAndPassword(ctx context.Context, name, password string) (e usermodel.Entity, err error) {
	if err = db.DBFromCtxValue(ctx, u.db).GetContext(ctx, &e, `SELECT id, name, password FROM t_user WHERE name = $1`, name); err != nil {
		err = errors.WithStack(err)
		return
	}
...
func (u *userImpl) Add(ctx context.Context, e usermodel.Entity) (id int, err error) {
	...
	if err = db.DBFromCtxValue(ctx, u.db).GetContext(ctx, &id, `INSERT INTO t_user (name, phone, email, password)
